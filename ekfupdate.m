...
for i = 1:size(z,2)  %%% LOOP TILL THE NUMBER OF LANDMARKS
    switch lower(Param.dataAssociation)
        ...
        case 'nn'
            initialize_new_landmark(z(:,i), Param.R);
            id_in_state = da_nn(z(1:3,i), Param.R); %%RETURNS THE ARRAY INDEX AT WHICH THE LANDMARK IS STORED
            if Param.sim==1
        ...
        del_x = State.Ekf.predMu(id_in_state - 2) - State.Ekf.predMu(1);
        del_y = State.Ekf.predMu(id_in_state-1) - State.Ekf.predMu(2);
        q = (del_x)^2 + (del_y)^2; 
        zhat_new = [sqrt(q); minimizedAngle(atan2(del_y, del_x) - State.Ekf.predMu(3)) ;z(3,i )];%%PREDICT THE OBSERVATION
        F_x_j=zeros(6,size(State.Ekf.predMu,1));
        F_x_j(1,1)= 1;
        F_x_j(2,2) = 1;
        F_x_j(3,3) = 1;
        F_x_j(4, id_in_state -2) = 1;
        F_x_j(5,id_in_state-1) = 1;
         F_x_j(6,id_in_state) = 1;
%             F_x_j(6,3+2*k) = 1;
         H_final=[-sqrt(q)*del_x,-sqrt(q)*del_y, 0, sqrt(q)*del_x,sqrt(q)*del_y 0;...
        ...
        State.Ekf.predMu = State.Ekf.predMu + K*(z(1:3,i)-zhat_new);    %%UPDATE MU
        State.Ekf.predSigma= (eye(size(K*H_final))-K*H_final)*State.Ekf.predSigma;      %%UPDATE SIGMA
%         State.Ekf.PG = [State.Ekf.PG;delx 2 (id_in_state-3)/2 ];
%         temp =[temp;id_in_state];
%         assignin('base','temp',temp);
        State.Ekf.temp=[State.Ekf.temp;id_in_state];
