...
sigma.d = 0.05; % [m] in diameter
Param.R = diag([sigma.r, sigma.beta, sigma.d].^2);
%===================================================

% Initialize State
%===================================================
% State.Ekf.mu = [Data.Gps.x(2), Data.Gps.y(2), 36*pi/180]';
State.Ekf.mu = [Data.Gps.x(2), Data.Gps.y(2), 0]';
...
State.Ekf.PG=[];
State.Ekf.temp = [];
% State.Ekf.LM = [State.Ekf.mu ones(3,1) ones(3,1) zeros(3,1) ];
global AAr;
AAr = [0:360]*pi/360;
...
for k=1:min(nSteps, length(Data.Laser.time))
    ...
    %State.Ekf.PF = [State.Ekf.PG;State.Ekf.mu(1:3) ones(3,1) k*ones(3,1) (k-1)*ones(3,1)];
    
    % Storing data for pose graph optimisation
   %State.Ekf.PG = [State.Ekf.PG;State.Ekf.mu(1:3) ones(3,1) k*ones(3,1) (k-1)*ones(3,1)]; 
   %NoLM = length(State.Ekf.temp);
   
   %check = State.Ekf.temp;
   %temp(1:3:NoLM*3,1:2:3)=[State.Ekf.predMu(check-2,1) (check-3)/3];
   %temp(2:3:NoLM*3,1:2:3)=[State.Ekf.predMu(check-1,1) (check-3)/3];
%    temp(2:2:NoLM*2,1:2:3)=[State.Ekf.mu(check,1) (check-3)/2];
%    temp(:,2)=2;
%    temp(:,4) = k*ones(size(temp,1),1);
%    State.Ekf.PG = [State.Ekf.PG;temp];
%    State.Ekf.temp=[];
%  clear temp
%     count = 1;
%     State.Ekf.PG = [State.Ekf.PG;State.Ekf.mu(1:3) ones(3,1) k*ones(3,1) (k-1)*ones(3,1)];
...
%         State.Ekf.PG = [State.Ekf.PG;x1 2 count k];
%         count = count+1;
%     end
%     State.Ekf.PG = [State.Ekf.PG;State.Ekf.predMu];
%     index(k,1)= length(State.Ekf.predMu)-3;
    doGraphics_Sarvesh(z);
    drawnow;
    if pauseLen > 0
        pause(pauseLen);
    end
