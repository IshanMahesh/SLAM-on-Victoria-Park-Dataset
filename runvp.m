...
for k=1:min(nSteps, length(Data.Laser.time))
    ...
    % Modified by Sarvesh to store each state output
%     NoLM = (length(State.Ekf.predMu)-3)/2;
%     temp = State.Ekf.predMu;
%     temp(1:3,2)=1;
%     temp(4:end,2)=2;
%     temp(1:3,3)=k;
%     temp(4:2:end,3)=(1:NoLM)';
%     temp(5:2:end,3)=(1:NoLM)';
%     temp(1:3,4)=k-1;
% %     temp(4:end,4) = k;
%     
%     
...
%     NoLM = (length(State.Ekf.predMu)-3)/2;
%     NState = State.Ekf.predMu;
%     NState(1:3,2)=1;
%     NState(4:end,2)=2;
%     NState(1:3,3)=k;
%     NState(4:2:end,1:2:3)=[NState(1,1)-NState(4:2:end,1) (1:NoLM)']
%     NState(5:2:end,1:2:3)=[NState(2,1)-NState(5:2:end,1) (1:NoLM)']
%     NState(1:3,4)=k-1;
%     NState(4:end,4)=k;
%     Sarvesh_Z = [Sarvesh_Z;NState];
%     LMX = State.Ekf.predMu(4:2:end);
%     LMY = State.Ekf.predMu(5:2:end);
%     RPX = State.Ekf.predMu(1)-LMX;
%     RPY = State.Ekf.predMu(2)-LMY;
    
%     Store_in_Z = 
%     Sarvesh_Z = [Sarvesh_Z;State.Ekf.predMu(1:3)]
    doGraphics(z);
    drawnow;
    if pauseLen > 0
        pause(pauseLen);
